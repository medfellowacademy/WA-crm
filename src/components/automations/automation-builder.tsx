...
const STEP_META: Record<AutomationStepType, StepMeta> = {
  ...
  ai_reply: { label: "AI Auto-reply", icon: Bot, border: "border-l-violet-500" },
}
...
function StepEditor({
  ...
  switch (step.step_type) {
    ...
    case "ai_reply":
      return (
        <p className="text-xs text-slate-400">
          Generates a context-aware reply using OpenAI and sends it to the contact.
          Configure the OpenAI API key and system prompt in{" "}
          <strong className="text-slate-300">Settings → Integrations</strong>.
        </p>
      )
    default:
      return null
...
function previewFor(step: BuilderStep): string {
  switch (step.step_type) {
    ...
    case "ai_reply":
      return "GPT generates reply from context"
    case "send_webhook":
