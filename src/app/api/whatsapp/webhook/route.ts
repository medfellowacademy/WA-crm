...
async function processMessage(
  ...
  if (orgId) {
    ...
    if (contactOutcome.wasCreated) {
      dispatchWebhookEvent(orgId, 'new_contact', {
        contact_id:   contactRecord.id,
        contact_name: contactRecord.name,
        contact_phone: contactRecord.phone,
      }).catch(err => console.error('[webhooks] new_contact dispatch failed:', err))
    }
  }
...
async function findOrCreateContact(
  ...
  // Look up existing contacts scoped by org (or user for backwards compat)
  const query = supabaseAdmin().from('contacts').select('*')
  const { data: contacts, error: contactsError } = orgId
